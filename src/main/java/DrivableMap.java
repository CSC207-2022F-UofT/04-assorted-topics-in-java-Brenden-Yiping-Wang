...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String identity, Drivable obj){
        if (this.drivable_map.containsKey(identity)){
            return false;
        }else{
            this.drivable_map.put(identity, obj);
            return true;
        }
    }
    ...
    public boolean hasFasterThan(int speed){
        for (Drivable v :this.drivable_map.values()){
    ...
    public List<Tradable> getTradable(){
        ArrayList<Tradable> temp = new ArrayList<>();
        for (Drivable v : this.drivable_map.values()){
